...
func ExtractDocument(sourceDoc string, raw []byte, tokenizerID string) (Document, error) {
	...
	if len(frontmatter) > 0 {
		nodes = append(nodes, newNode(len(nodes), NodeYAMLFrontmatter, string(bytes.TrimSpace(frontmatter)), false, ModalityNone))
	}
...
func collectNodes(source []byte, node ast.Node, frame *DeonticFrame, out *[]Node) {
	...
	case *ast.List:
		for item := n.FirstChild(); item != nil; item = item.NextSibling() {
			...
			if textValue != "" {
				*out = append(*out, newNode(len(*out), NodeList, textValue, false, ModalityNone))
			}
		}
	case *ast.Blockquote:
		textValue := normalizeInlineText(collectText(source, n))
		if textValue != "" {
			*out = append(*out, newNode(len(*out), NodeBlockquote, textValue, false, ModalityNone))
		}
	case *ast.Paragraph:
		textValue := normalizeInlineText(collectText(source, n))
		if textValue != "" {
			eval := frame.EvaluateText([]byte(textValue))
			*out = append(*out, newNode(len(*out), NodeParagraph, textValue, eval.Promoted, eval.Mask))
		}
	case *ast.Heading:
		textValue := normalizeInlineText(collectText(source, n))
		if textValue != "" {
			*out = append(*out, newNode(len(*out), NodeHeading, textValue, false, ModalityNone))
		}
	case *ast.FencedCodeBlock:
		textValue := normalizeBlockText(collectLines(source, n.Lines()))
		if textValue != "" {
			*out = append(*out, newNode(len(*out), NodeCodeBlock, textValue, false, ModalityNone))
		}
	case *ast.CodeBlock:
		textValue := normalizeBlockText(collectLines(source, n.Lines()))
		if textValue != "" {
	...
	case *ast.HTMLBlock:
		textValue := normalizeBlockText(collectLines(source, n.Lines()))
		if textValue != "" {
			*out = append(*out, newNode(len(*out), NodeHTMLBlock, textValue, false, ModalityNone))
		}
	default:
...
func newNode(ordinal int, kind NodeKind, textValue string, promoted bool, mask ModalityMask) Node {
	return Node{
		Ordinal:       ordinal,
		Kind:          kind,
